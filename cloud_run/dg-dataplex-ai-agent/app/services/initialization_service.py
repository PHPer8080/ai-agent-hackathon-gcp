...
class InitializationService:
    ...
    def __init__(self):
        ...
        self.dataplex_service = DataplexService()
        self.agent = None
        self.runner = None
    ...
    def _create_agent(self) -> LlmAgent:
        ...
        # Dataplex データ品質ツールの作成
        dataplex_tools = []
        if self.config.mcp_server_url:
            dataplex_tools = [FunctionTool(self.dataplex_service.suggest_data_quality_rules)]
            logger.info(f"Dataplex FunctionTool を作成: {len(dataplex_tools)}個")

        return LlmAgent(
            name="dg_dataplex_assistant",
            model=AgentConfig.MODEL_NAME,
            instruction=instruction,
            description="Google Cloud Dataplexデータアシスタント（ガードレール機能付き）",
            tools=dataplex_tools,
            before_tool_callback=self.dataplex_service.combined_guardrail_callback,  # 🛡️ ガードレール設定
        )
