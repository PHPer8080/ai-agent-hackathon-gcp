...
SERVICE_NAME="dg-bigquery-ai-agent"
REGION="asia-northeast1"
IMAGE_REPOSITORY="${REGION}-docker.pkg.dev/${PROJECT_ID}/cloud-run-source-deploy"

# 依存サービスの内部エンドポイント（describe取得）
MCP_SERVER_URL=$(gcloud run services describe dg-bigquery-mcp-server --platform managed --region "${REGION}" --project="${PROJECT_ID}" --format 'value(status.url)' 2>/dev/null || echo "")
